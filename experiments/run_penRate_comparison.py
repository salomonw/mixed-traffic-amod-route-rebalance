...
def penRate(netFile, gFile, fcoeffs, alg='CARS3', rebalancing=True, xa=1):
    ...
    for penetration_rate in np.linspace(0.01, 0.99, 11):
        ...
        tNet_cavs.build_supergraph(walk_multiplier=1)#3.226)
        tNet_non_cavs.build_supergraph(walk_multiplier=0.0001)
        ...
        for i in range(20):
            ...
            if alg == 'CARS':
                ...
                if i == 0:
                    cars.solveMSAsocialCARS(tNet_cavs, exogenous_G=0)
...
lstyle = ['-', '--', ':']
i=0
for alg in ['CARS3']:# 'disjoint']:#, 'CARS2', 'disjoint']:
