...
## Define Objective
@NLobjective(m, Min, sum( (x[link]+G_exogenous[link]["flow"])* G[link]["t_0"]* sum( fcoeffs[n]*((x[link] + G_exogenous[link]["flow"])/G[link]["capacity"])^(n-1) for n=1:N ) for link in links))
